...
namespace ISM3025.Features
{
    public static class DynamicLogos
    {
        ...
        {
            //var factions = Enum.GetValues(typeof(FactionValue));
            var factions = FactionEnumeration.FactionList;
            var logos = new Dictionary<FactionValue, GameObject>();

            foreach (FactionValue faction in factions)
            {
                string factionName;
                if (faction.FactionDefID.StartsWith("faction_"))
                {
                    factionName = faction.FactionDefID.Substring(8);
                }
                else
                {
                    factionName = faction.FactionDefID;
                }
                if (factionName == null)
                    continue;
